...
def initialiser_base():
    ...
    cur.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS PARTICIPATIONS (
            ...
            NB_COUPS_JOUEUR INTEGER NOT NULL DEFAULT 0,
    ...
    # Migration : ajoute NB_COUPS_JOUEUR si la base existait avant cette version
    colonnes = [row[1] for row in cur.execute("PRAGMA table_info(PARTICIPATIONS)")]
    if 'NB_COUPS_JOUEUR' not in colonnes:
        cur.execute("ALTER TABLE PARTICIPATIONS ADD COLUMN NB_COUPS_JOUEUR INTEGER NOT NULL DEFAULT 0")
        conn.commit()
        print("✅ Migration : colonne NB_COUPS_JOUEUR ajoutée à PARTICIPATIONS")
...
def sauvegarder_partie(partie):
    ...
    try:
        ...
        # 3. Participations (avec nb coups personnels)
        nb_coups_j1 = partie.joueur1.nb_coups
        nb_coups_j2 = partie.joueur2.nb_coups
        cur.execute(
            "INSERT INTO PARTICIPATIONS (PARTIE_ID, JOUEUR_ID, COULEUR, SCORE_FINAL, NB_COUPS_JOUEUR) "
            "VALUES (?, ?, ?, ?, ?)",
            (partie_id, id_j1, partie.joueur1.couleur.value, partie.joueur1.score, nb_coups_j1)
        )
        cur.execute(
            "INSERT INTO PARTICIPATIONS (PARTIE_ID, JOUEUR_ID, COULEUR, SCORE_FINAL, NB_COUPS_JOUEUR) "
            "VALUES (?, ?, ?, ?, ?)",
            (partie_id, id_j2, partie.joueur2.couleur.value, partie.joueur2.score, nb_coups_j2)
        )
...
def obtenir_top5_gagnants():
    """
    Retourne le top 5 des gagnants ayant remporté une partie
    avec le moins de coups personnels.

    Returns:
        list[dict]: Liste de dicts avec clés 'nom', 'nb_coups', 'date'
                    triée par nb_coups_joueur ASC, limitée à 5 entrées.
    """
    conn = _connexion()
    cur  = conn.cursor()
    cur.execute("""
        SELECT j.NOM, pa.NB_COUPS_JOUEUR, p.DATE_DEBUT
        FROM PARTICIPATIONS pa
        JOIN JOUEURS j  ON j.JOUEUR_ID  = pa.JOUEUR_ID
        JOIN PARTIES p  ON p.PARTIE_ID  = pa.PARTIE_ID
        WHERE p.GAGNANT_ID = pa.JOUEUR_ID
          AND pa.NB_COUPS_JOUEUR > 0
        ORDER BY pa.NB_COUPS_JOUEUR ASC
        LIMIT 5
    """)
    rows = cur.fetchall()
    conn.close()
    return [{'nom': r[0], 'nb_coups': r[1], 'date': r[2]} for r in rows]
    ...
    if participations:
        ...
        for pa in participations:
            if pa[1] != partie_courante:
                ...
                lignes.append(f"-- Partie {pa[1]} : {pa[6]} (rouge) vs {pa[7]} (bleu)")
            lignes.append(
                f"insert into PARTICIPATIONS values ({pa[0]}, {pa[1]}, {pa[2]}, '{pa[3]}', {pa[4]}, {pa[5]});"
            )
