...
from bouncebox_db import initialiser_base, obtenir_top5_gagnants, sauvegarder_partie
...
class Afficheur:
    ...
    def afficher_ecran_titre(self, screen, largeur, hauteur):
        ...
        # Logo centré (même composant que pendant le jeu, échelle plus grande)
        self.dessiner_logo(screen, largeur // 2, hauteur // 2 - 130, echelle=1.4)

        surf_sous = self.font_moyen.render("Jeu de billard a deux joueurs", True, GRIS_CLAIR)
        screen.blit(surf_sous, (largeur // 2 - surf_sous.get_width() // 2, hauteur // 2 - 50))

        btn = pygame.Rect(largeur // 2 - 120, hauteur // 2 + 20, 240, 55)
        souris = pygame.mouse.get_pos()
        self.dessiner_bouton(screen, "Jouer", btn, survol=btn.collidepoint(souris))

        # ── Top 5 gagnants (côté droit) ──────────────────────────────
        top5 = obtenir_top5_gagnants()
        panel_w = 240
        panel_x = largeur - panel_w - 20
        panel_y = 40

        titre_surf = self.font_moyen.render("Meilleurs gagnants", True, OR)
        screen.blit(titre_surf, (panel_x, panel_y))

        col_nom  = panel_x
        col_coup = panel_x + panel_w - 60
        ent_nom  = self.font_petit.render("Joueur", True, GRIS_CLAIR)
        ent_coup = self.font_petit.render("Coups", True, GRIS_CLAIR)
        screen.blit(ent_nom,  (col_nom,  panel_y + 34))
        screen.blit(ent_coup, (col_coup, panel_y + 34))
        pygame.draw.line(screen, GRIS_CLAIR,
                         (panel_x, panel_y + 52), (panel_x + panel_w, panel_y + 52), 1)

        if top5:
            medailles = ["1.", "2.", "3.", "4.", "5."]
            for rang, entree in enumerate(top5):
                y = panel_y + 60 + rang * 28
                surf_rang = self.font_petit.render(
                    f"{medailles[rang]}  {entree['nom'][:16]}", True, BLANC)
                surf_coup = self.font_petit.render(str(entree['nb_coups']), True, OR)
                screen.blit(surf_rang, (col_nom, y))
                screen.blit(surf_coup, (col_coup + (60 - surf_coup.get_width()) // 2, y))
        else:
            vide = self.font_petit.render("Aucune partie sauvegardee", True, GRIS_CLAIR)
            screen.blit(vide, (panel_x, panel_y + 60))
    ...
    def afficher_ecran_mode(self, screen, largeur, hauteur):
        ...
        rect_retour = pygame.Rect(cx - 100, 470, 200, 50)
    ...
    def afficher_ecran_noms(self, screen, largeur, hauteur, noms, champ_actif, sauvegarder=False, mode='jvj', niveau_ia=NiveauIA.MOYEN):
        ...
        btn = pygame.Rect(cx - 140, 485, 280, 55)
        souris = pygame.mouse.get_pos()
    ...
    def _dessiner_bille_brillante(self, screen, centre, rayon, couleur):
        ...
        # Ombre portée gris foncé identique pour toutes les boules
        ombre_offset = max(2, rayon // 4)
        pygame.draw.circle(screen, (30, 30, 30),
                           (cx + ombre_offset, cy + ombre_offset), rayon)

        # Cercle de base opaque centré sur la boule — couvre entièrement l'ombre
        # pour qu'aucune zone grise ne reste visible autour de la boule
        couleur_base = (int(r * 0.65), int(g * 0.65), int(b * 0.65))
        pygame.draw.circle(screen, couleur_base, (cx, cy), rayon)

        # Dégradé sphérique par-dessus
        n = max(3, rayon // 2)
    ...
    def afficher_interface(self, screen, partie, force_actuelle=0.0):
        ...
        # Panneau joueur actif en haut, timer au centre, joueur inactif en bas
        hauteur_zone = self.hauteur_ecran
        panneau_h = 78
        timer_h = 60

        total_h = panneau_h + timer_h + panneau_h
        y_depart = (hauteur_zone - total_h) // 2
    ...
    def gerer_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                rect_j1 = pygame.Rect(cx - 160, 200, 320, 44)
                rect_j2 = pygame.Rect(cx - 160, 310, 320, 44)
                btn_lancer = pygame.Rect(cx - 140, 485, 280, 55)
